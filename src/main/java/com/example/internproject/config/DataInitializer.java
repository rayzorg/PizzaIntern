...
public class DataInitializer {
	...
	@Bean
	public CommandLineRunner loadMenu(PizzaService pizzaService, ToppingService toppingService,
			AuthService authService) {

		return args -> {

			if (pizzaService.getAll().size() == 0) {
				//users
				authService.register("customer", "customer@customer.com", "1234", "frfrfrfrfr", Role.CUSTOMER);
				authService.register("admin", "admin@admin.com", "1234", "055604859", Role.ADMIN);
				//Toppings
				...
				//Pizzas
				Pizza margherita = new Pizza("Margherita", "Classic cheese pizza", new BigDecimal("8.50"),
						"pizzas/margherita.jpg");
				margherita.setToppings(Set.of(cheese));
				pizzaService.addPizza(margherita);

				Pizza pepperoniPizza = new Pizza("Pepperoni", "Spicy pepperoni pizza", new BigDecimal("10.00"),
						"pizzas/pepperoni.jpg");
				pepperoniPizza.setToppings(Set.of(cheese, pepperoni));
				pizzaService.addPizza(pepperoniPizza);

				Pizza hawaiian = new Pizza("Hawaiian", "Ham and pineapple", new BigDecimal("9.50"), "pizzas/hawai.jpg");
				hawaiian.setToppings(Set.of(cheese, ham, pineapple));
				pizzaService.addPizza(hawaiian);

				Pizza mushroomDelight = new Pizza("Mushroom Delight", "Cheese and fresh mushrooms",
						new BigDecimal("9.00"), "pizzas/mushrooms.jpg");
				mushroomDelight.setToppings(Set.of(cheese, mushroom));
				pizzaService.addPizza(mushroomDelight);

				Pizza meatLovers = new Pizza("Meat Lovers", "Pepperoni, bacon, chicken", new BigDecimal("12.00"),
						"pizzas/meat.jpg");
				meatLovers.setToppings(Set.of(pepperoni, bacon, chicken));
				pizzaService.addPizza(meatLovers);

				Pizza veggie = new Pizza("Veggie", "Mushroom, onions, olives", new BigDecimal("10.00"),
						"pizzas/veggie.jpg");
				veggie.setToppings(Set.of(mushroom, onions, olives));
				pizzaService.addPizza(veggie);

				Pizza bbqChicken = new Pizza("BBQ Chicken", "Chicken, BBQ sauce, onions", new BigDecimal("11.50"),
						"pizzas/bbqchicken.jpg");
				bbqChicken.setToppings(Set.of(chicken, bbq, onions));
				pizzaService.addPizza(bbqChicken);

				Pizza fourCheese = new Pizza("Four Cheese", "Mozzarella, cheddar, parmesan, gorgonzola",
						new BigDecimal("11.00"), "pizzas/fourcheese.jpg");
				fourCheese.setToppings(Set.of(mozzarella, cheddar, parmesan, gorgonzola));
				pizzaService.addPizza(fourCheese);

				Pizza baconSupreme = new Pizza("Bacon Supreme", "Bacon, cheese, onions", new BigDecimal("11.00"),
						"pizzas/bacon.jpg");
